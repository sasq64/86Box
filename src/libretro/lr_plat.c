...
{
    snprintf(s, size, "%s%s", lr_base_path, EMU_NAME);
}
...
plat_get_ticks_common(void)
{
    ...
    if (first_use) {
        ...
        starting_time = plat_timer_read();
        first_use     = 0;
    }
    ending_time          = plat_timer_read();
...
plat_delay_ms(uint32_t count)
{
    ...
    ts.tv_sec  = count / 1000;
    ts.tv_nsec = (long) (count % 1000) * 1000000L;
    nanosleep(&ts, NULL);
}
